...
beforeAll(async () => {
  ...
  await db.query(
    "insert into assets(id,market_hash_name) values ($1,'Glove Case')",
    [asset],
  );
  await db.query(
    "insert into collector_runs(id,source,window_start,started_at,claim_key) values ($1,'SKINPORT',now(),now(),'window1')",
    [run],
  );
});
afterAll(async () => {
...
it("applies real migrations, exact numeric storage, append-only trigger, uniqueness and rollback", async () => {
  ...
  expect(
    (
      await db.query<{ min_price: string }>(
        "select min_price from market_observations",
      )
    ).rows[0].min_price,
  ).toBe("999999999999.12345678");
  await expect(db.query(insert, [asset, run])).rejects.toThrow();
  await expect(
    db.query("update market_observations set quantity=1"),
  ).rejects.toThrow("append-only");
  await expect(db.query("delete from market_observations")).rejects.toThrow(
    "append-only",
  );
  await expect(db.query("truncate market_observations")).rejects.toThrow(
    "append-only",
  );
  await expect(
    db.query(
      "insert into collector_runs(source,window_start,started_at,claim_key) values ('SKINPORT',now(),now(),'window1')",
    ),
  ).rejects.toThrow();
  await expect(
    db.transaction(async (tx) => {
      await tx.query("update collector_runs set status='SUCCESS' where id=$1", [
        run,
      ]);
  ...
  expect(
    (
      await db.query<{ status: string }>(
        "select status from collector_runs where id=$1",
        [run],
      )
    ).rows[0].status,
  ).toBe("RUNNING");
...
it("executes health and analytical SQL with real rows and missing windows", async () => {
  ...
  await db.query(
    "update collector_runs set status='SUCCESS', tracked_assets=1, items_matched=1, observations_inserted=1, duration_ms=100 where id=$1",
    [run],
  );
  await db.query(
  ...
  expect(health.observations).toMatchObject({
    last24h: 2,
...
it("completion telemetry stamps only terminal runs and remains stable on retries", async () => {
  ...
  for (const [id, status] of [
    ...
    await db.query(
      "insert into collector_runs(id,source,window_start,started_at,status) values ($1,'SKINPORT',now(),now(),$2)",
      [id, status],
    );
  }
  // The update builder is compatible across the HTTP and embedded PostgreSQL drivers.
  const store = collectorStore(
    drizzle(db) as unknown as Parameters<typeof collectorStore>[0],
  );
  const timing = {
    finishedAt: new Date("2026-09-09T09:00:02Z"),
    durationMs: 2000,
  };
  await store.completeTiming(pending, timing);
  expect(
    (
      await db.query<{ finished_at: unknown }>(
        "select finished_at from collector_runs where id=$1",
        [pending],
      )
    ).rows[0].finished_at,
  ).toBeNull();
  await store.completeTiming(completed, timing);
  await store.completeTiming(completed, {
    finishedAt: new Date("2026-09-09T09:00:09Z"),
    durationMs: 9000,
  });
  const row = (
    await db.query<{ finished_at: Date; duration_ms: number }>(
      "select finished_at,duration_ms from collector_runs where id=$1",
      [completed],
    )
  ).rows[0];
