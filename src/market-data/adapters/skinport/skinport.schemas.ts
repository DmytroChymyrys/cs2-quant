...
const decimalToken =
  z.custom<import("lossless-json").LosslessNumber>(isLosslessNumber);
export const price = decimalToken
  .transform((v) => v.value)
  .refine((v) => {
  ...
  }, "Price outside numeric(20,8)")
  .transform((v) => new Decimal(v).toFixed(8))
  .nullable();
const integerToken = decimalToken.refine(
  (v) => new Decimal(v.value).isInteger(),
  "Expected integer",
);
const count = integerToken
  .transform((v) => Number(v.value))
  .pipe(z.number().int().min(0).max(2147483647));
const epoch = integerToken
  .transform((v) => Number(v.value))
  .pipe(z.number().int().min(0).max(8640000000000));
const identity = {
...
const periodSchema = z
  .object({ min: price, max: price, avg: price, median: price, volume: count })
  .passthrough();
export const historySchema = z
  .object({
    ...identity,
    version: z.string().nullable().optional(),
    last_24_hours: periodSchema,
    last_7_days: periodSchema,
    last_30_days: periodSchema,
    last_90_days: periodSchema,
  })
  .passthrough();
